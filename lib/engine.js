...
import { getFoto, FOTOS } from './fotos.js';
import { lootSpel } from './loting.js';
...
/**
 * Alle puntenwaarden op één plek, zodat je eraan kan draaien zonder de
 * rest van de code te doorzoeken.
 *
 * Over de strafpunten: die gelden alleen waar blijven gokken je iets
 * kán opleveren, namelijk in de rondes waar je mag doorraden tot het
 * juist is. Daar telt elke misgok apart, met een bodem zodat één vraag
 * niemand de das omdoet.
 *
 * In de andere rondes staat er geen straf op een fout antwoord. Je kiest
 * daar één keer uit vier vakjes, of alleen je laatste antwoord telt —
 * spammen levert dus niets op. Er wél straf op zetten zou enkel eerlijk
 * twijfelen afstraffen, en mensen doen zwijgen als ze het niet zeker
 * weten. Net het tegenovergestelde van wat je op een familiefeest wil.
 */
export const PUNTEN = {
  basis: { mc: 100, woord: 100, truefalse: 60, open: 120, geo: 120 },
  fout: {}, // geen straf buiten de doorraadrondes — zie hierboven

  misgok: -15,      // per foute gok bij uitzoomen, uitbeelden en tekenen
  misgokBodem: -60, // verder dan dit zakt één vraag niet

  schatten: [120, 90, 70, 50],
  schattenRest: 30,
  schattenExact: 50,
  ...
  raders: [150, 110, 80],
  radersRest: 50,
  perRader: 60,     // wat de tekenaar of uitbeelder per radend team krijgt
};

const BASIS = PUNTEN.basis;
...
// Rondes waar je blijft raden tot je het juist hebt: een misser mag de
// vraag niet afsluiten, want het beeld gaat nog open en de speler die
// uitbeeldt is nog bezig.
const IS_DOORRADEN = (type) => IS_OPVOERING(type) || type === 'zoom';
...
// Wat de misgokken van een team gekost hebben, met een bodem eronder.
function straf(inzending) {
  const aantal = inzending?.mis || 0;
  if (!aantal) return 0;
  return Math.max(PUNTEN.misgokBodem, aantal * PUNTEN.misgok);
}

function strafTekst(inzending) {
  const verlies = straf(inzending);
  if (!verlies) return '';
  const aantal = inzending.mis;
  return ` — ${aantal} misgok${aantal === 1 ? '' : 'ken'} (${verlies})`;
}
...
export function berekenPunten(ronde, vraag, inzendingen, vraagStart, teams) {
  ...
  if (ronde.type === 'estimate') {
    ...
    gerangschikt.forEach((rij, i) => {
      let punten = PUNTEN.schatten[i] ?? PUNTEN.schattenRest;
      if (rij.afwijking === 0) punten += PUNTEN.schattenExact;
      uitslag[rij.teamId] = {
        punten,
  ...
  if (ronde.type === 'zoom') {
    // Hier telt de volgorde: wie het eerst door heeft wat er staat,
    // pakt het meest. Het beeld gaat immers voor iedereen even traag open.
    const geraden = Object.entries(inzendingen)
      .filter(([, inzending]) => isGoedAntwoord(inzending.waarde, vraag))
      .sort((a, b) => a[1].at - b[1].at);

    geraden.forEach(([teamId, inzending], i) => {
      const s = Math.round(secondenSinds(vraagStart, inzending.at));
      uitslag[teamId] = {
        punten: (PUNTEN.zoom[i] ?? PUNTEN.zoomRest) + straf(inzending),
        goed: true,
        toelichting: `${i === 0 ? 'Als eerste' : `${i + 1}e`}, na ${s} sec${strafTekst(inzending)}`,
      };
    });

    for (const [teamId, inzending] of Object.entries(inzendingen)) {
      if (!uitslag[teamId]) {
        uitslag[teamId] = { punten: straf(inzending), goed: false, toelichting: String(inzending.waarde) };
      }
    }
    return uitslag;
  }
  ...
  if (IS_OPVOERING(ronde.type)) {
    ...
    raders.forEach(([teamId, inzending], i) => {
      uitslag[teamId] = {
        punten: (PUNTEN.raders[i] ?? PUNTEN.radersRest) + straf(inzending),
        goed: true,
        toelichting: (i === 0 ? 'Als eerste geraden!' : 'Geraden') + strafTekst(inzending),
      };
    });
    for (const [teamId, inzending] of Object.entries(inzendingen)) {
      if (!uitslag[teamId]) {
    ...
    if (vraag._opvoerTeam) {
      const punten = raders.length * PUNTEN.perRader;
  ...
  for (const [teamId, inzending] of Object.entries(inzendingen)) {
    ...
    // Wie niets instuurt verliest niets; wie ernaast zit wel een beetje.
    uitslag[teamId] = {
      punten: goed ? basis + snelheidsbonus(basis, s, limiet) : (PUNTEN.fout[ronde.type] ?? 0),
      goed,
...
export function nieuwSpel(code) {
  return {
    ...
    aangemaakt: Date.now(),
    versie: 1,
    // Welke vragen deze quiz krijgt, geloot bij het aanmaken zodat de
    // volgorde vastligt en elke speler dezelfde vraag ziet.
    loting: lootSpel(),
    fase: 'lobby',
    rondeIndex: 0,
    vraagIndex: 0,
...
// De vraagnummers die deze quiz uit de voorraad getrokken heeft. Oudere
// spellen zonder loting spelen gewoon de hele ronde.
function gelooteVragen(spel, ronde) {
  if (!ronde) return [];
  const gekozen = spel.loting?.[ronde.id];
  if (Array.isArray(gekozen) && gekozen.length) return gekozen;
  return ronde.vragen.map((_, i) => i);
}

function aantalVragen(spel, ronde = huidigeRonde(spel)) {
  return gelooteVragen(spel, ronde).length;
}

function huidigeVraag(spel) {
  const ronde = huidigeRonde(spel);
  if (!ronde) return null;
  const nummers = gelooteVragen(spel, ronde);
  return ronde.vragen[nummers[spel.vraagIndex]] || null;
}
...
// Een teamfoto is een klein vierkant jpeg als data-URI. We houden hem
// bewust piepklein: hij zit in de spelstand en die gaat bij elk antwoord
// heen en weer naar de database.
const MAX_FOTO = 24 * 1024;

export function schoneFoto(waarde) {
  if (typeof waarde !== 'string') return null;
  if (!/^data:image\/(jpeg|png|webp);base64,[A-Za-z0-9+/=]+$/.test(waarde)) return null;
  if (waarde.length > MAX_FOTO) return null;
  return waarde;
}

export function zetTeamFoto(spel, spelerId, foto) {
  const team = spel.teams.find((t) => t.leden.some((l) => l.id === spelerId));
  if (!team) return { fout: 'Je zit nog in geen enkel team.' };

  const schoon = schoneFoto(foto);
  if (foto && !schoon) return { fout: 'Die foto kon ik niet verwerken. Probeer het opnieuw.' };

  team.foto = schoon; // null wist de foto weer
  return { ok: true };
}

export function voegTeamToe(spel, naam, foto) {
...
export function neemAntwoord(spel, teamId, waarde, spelerId) {
  ...
  // De klok bepaalt de snelheidsbonus en de spanning, maar sluit de
  // vraag niet af. Wie traag is verliest punten, geen beurt — anders
  // mist oma een vraag omdat ze de knop nog zocht.
  ...
  if (IS_DOORRADEN(ronde?.type) && bestaand?.vast) {
    return { fout: 'Jullie antwoord staat al vast.' };
  }

  // Bij uitbeelden en tekenen is de opdracht zelf het juiste antwoord;
  // bij het uitzoomen staat het gewoon in de vraag.
  const vraag = huidigeVraag(spel);
  let juist = false;
  if (vraag && IS_DOORRADEN(ronde?.type)) {
    juist = isGoedAntwoord(waarde, IS_OPVOERING(ronde.type) ? opvoerAntwoord(vraag) : vraag);
  }

  // In de doorraadrondes tellen we hoe vaak een team ernaast zat, want
  // daar kost elke misgok punten.
  const misVoor = bestaand?.mis || 0;
  const nieuweMis = IS_DOORRADEN(ronde?.type) && !juist ? misVoor + 1 : misVoor;

  // We onthouden wie het insturen deed. Een team heeft één antwoord, en
  // zonder die naam overschrijven ploegmaats elkaar in stilte — iemand
  // die het juist had, ziet dan niet dat een ander het weggeklikt heeft.
  const lid = team.leden.find((l) => l.id === spelerId);
...
export function neemTekening(spel, spelerId, strepen, wissen) {
  ...
  for (const streep of strepen || []) {
    ...
    // Het volgnummer zegt waar de lijn hoort. De gsm stuurt de lijn die
    // nog getrokken wordt telkens opnieuw, met dezelfde plek, zodat hij
    // aangroeit in plaats van dubbel te komen.
    const plek = Number.isInteger(streep.i) && streep.i >= 0 && streep.i <= spel.tekening.length
      ? streep.i
      : spel.tekening.length;

    // Een grens op de omvang, zodat een enthousiaste krabbelaar de quiz
    // niet traag maakt.
    if (plek >= 300) continue;
    spel.tekening[plek] = nieuw;
  }
  return { ok: true };
}
...
function naVraag(spel) {
  ...
  if (spel.vraagIndex + 1 < aantalVragen(spel, ronde)) {
    spel.vraagIndex += 1;
    openVraag(spel);
    return;
  }
...
function vraagVoorScherm(spel, ronde, vraag, toonAntwoord) {
  ...
  const basis = {
    ...
    totaal: aantalVragen(spel, ronde),
    type: ronde.type,
    seconden: ronde.seconden,
    niveau: vraag.niveau || 'gemiddeld',
  };

  if (ronde.type === 'mc') {
    return { ...basis, tekst: vraag.q, domein: vraag.domein || null, opties: vraag.opties, antwoord: toonAntwoord ? vraag.antwoord : null, weetje: toonAntwoord ? vraag.weetje : null };
  }
  if (ronde.type === 'woord') {
  ...
  if (ronde.type === 'geo' || ronde.type === 'zoom') {
    const foto = getFoto(vraag.foto);
    return {
      ...basis,
      foto: foto?.bestand || null,
      // De bronvermelding tonen we pas bij het antwoord, zodat ze de
      // vraag zelf niet weggeeft of het beeld ontsiert.
      bron: toonAntwoord && foto ? { maker: foto.maker, licentie: foto.licentie, pagina: foto.pagina } : null,
      plek: toonAntwoord ? vraag.plek || null : null,
      antwoord: toonAntwoord ? vraag.antwoord : null,
    };
  }
  if (IS_OPVOERING(ronde.type)) {
...
// Welke teams mogen deze vraag beantwoorden? Bij uitbeelden en tekenen
// doet het team dat aan de beurt is niet mee.
function mogenAntwoorden(spel, ronde) {
  const opvoering = IS_OPVOERING(ronde?.type);
  return spel.teams.filter((t) => !(opvoering && spel.uitbeelder?.teamId === t.id));
}

/**
 * Is iedereen binnen? Dan mag het groot scherm vanzelf doorgaan.
 *
 * Bij uitbeelden en tekenen telt alleen een júist antwoord: een team dat
 * er één keer naast gokt is nog niet klaar, anders zou de ronde meteen
 * stoppen bij de eerste misser.
 */
function iedereenKlaar(spel, ronde) {
  if (spel.fase !== 'vraag' || !spel.vraagOpen) return false;

  const inzendingen = spel.antwoorden[sleutel(spel.rondeIndex, spel.vraagIndex)] || {};
  const moeten = mogenAntwoorden(spel, ronde);
  if (!moeten.length) return false;

  const doorraden = IS_DOORRADEN(ronde?.type);
  return moeten.every((t) => {
    const inzending = inzendingen[t.id];
    if (!inzending) return false;
    return doorraden ? !!inzending.vast : true;
  });
}
...
function uitslagVoorScherm(spel) {
  ...
  return spel.teams
    .map((t) => {
      ...
      if (!r) return null;
      return { id: t.id, naam: t.naam, emoji: t.emoji, kleur: t.kleur, foto: t.foto || null, punten: r.punten, goed: r.goed, toelichting: r.toelichting };
    })
    .filter(Boolean)
    .sort((a, b) => b.punten - a.punten);
}

function fotosVanDitSpel(spel) {
  const paden = [];
  for (const ronde of rondes) {
    if (ronde.type !== 'geo' && ronde.type !== 'zoom') continue;
    for (const nummer of gelooteVragen(spel, ronde)) {
      const foto = getFoto(ronde.vragen[nummer]?.foto);
      if (foto) paden.push(foto.bestand);
    }
  }
  return paden;
}
...
export function hostBeeld(spel) {
  ...
  return {
    ...
    fase: spel.fase,
    titel: QUIZ_TITEL,
    ronde: ronde ? { id: ronde.id, naam: ronde.naam, uitleg: ronde.uitleg, regels: ronde.regels || [], icoon: ronde.icoon, type: ronde.type, nummer: spel.rondeIndex + 1, totaal: rondes.length } : null,
    vraag: vraagVoorScherm(spel, ronde, vraag, toon),
    vraagStart: spel.vraagStart,
    vraagOpen: spel.vraagOpen,
    antwoordenBinnen: binnen,
    verwachteAntwoorden: mogenAntwoorden(spel, ronde).length,
    iedereenKlaar: iedereenKlaar(spel, ronde),
    ...
    // In de lobby heeft het groot scherm niets te doen: mooi moment om
    // de foto's van déze quiz alvast op te halen, zodat er later geen
    // gat valt. Alleen de gelote foto's, niet de hele voorraad.
    fotosVooraf: spel.fase === 'lobby' ? fotosVanDitSpel(spel) : null,
    stand: stand(spel),
  };
}
...
export function spelerBeeld(spel, spelerId) {
  ...
  return {
    ...
    fase: spel.fase,
    titel: QUIZ_TITEL,
    team: team ? { id: team.id, naam: team.naam, emoji: team.emoji, kleur: team.kleur, foto: team.foto || null, score: team.score, plaats: eigenStand?.plaats ?? null } : null,
    teams: spel.teams.map((t) => ({ id: t.id, naam: t.naam, emoji: t.emoji, kleur: t.kleur, foto: t.foto || null, aantal: t.leden.length })),
    ronde: ronde ? { id: ronde.id, naam: ronde.naam, uitleg: ronde.uitleg, regels: ronde.regels || [], icoon: ronde.icoon, type: ronde.type } : null,
    vraag: vraagVoorScherm(spel, ronde, vraag, toon),
    vraagStart: spel.vraagStart,
    vraagOpen: spel.vraagOpen,
    antwoord: eigenAntwoord
      ? {
          waarde: eigenAntwoord.waarde,
          vast: !!eigenAntwoord.vast,
          mis: eigenAntwoord.mis || 0,
          door: eigenAntwoord.door || null,
          // Heeft een ploegmaat dit ingestuurd, of jij zelf?
          vanMij: eigenAntwoord.doorId ? eigenAntwoord.doorId === spelerId : null,
        }
      : null,
    beeldtUit,
    isDeUitbeelder,
